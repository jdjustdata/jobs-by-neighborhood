...
db_name = "jobs_by_neighborhood.db"
db_path = getcwd() + "/" + db_name

jobs_header = '''CREATE TABLE 'Jobs'
(Title TEXT, Company TEXT, Address TEXT, Neighborhood TEXT, Shift TEXT, Description INT, PostedDate DATE)'''
...
def add_job(location):
    conn = sqlite3.connect('jobs_by_neighborhood.db')
    c = conn.cursor()
    ...
    job = [title, company, address, neighborhood, shift, description, posted_date]
